...
%% Read component Data
failDatTable = readtable([dataPath,'/ComponentFailureData.xlsx'], 'Range', 'G1:M12');
repairRateTable = readtable([dataPath,'/RepairRateData.xlsx'], 'Range', 'A1:B5');
InitMarking = readmatrix([dataPath,'/InitialMarking.xlsx'], 'Range', 'A2:A14');
NPhases = 1;
NSubnets = 0;

%% Read A matrices, place IDs and transition IDs
P=1;
A.Ain{P} = readmatrix([dataPath,'/AinMatrix.xlsx'],'Range','C3:AC32');
A.Aout{P} = readmatrix([dataPath,'/AoutMatrix.xlsx'],'Range','C3:AC32');
A.pIds{P} = readmatrix([dataPath,'/AinMatrix.xlsx'],'Range','C2:AC2');
A.tIds{P} = readmatrix([dataPath,'/AoutMatrix.xlsx'],'Range','B3:B32');
...
for P=1:NPhases
    if any(A.Ain{P}>0)
        error('Output arcs declared in AinMatrix')
    elseif any(A.Aout{P}<0)
        error('Input arcs declared in AoutMatrix')
    end
